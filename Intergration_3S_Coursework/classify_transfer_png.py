...
path1 = 'colored_predicted_image_2022.png'
path2 = 'colored_predicted_image_2023.png'
...
for category in categories:
    Image.fromarray(change_images[category]).save(f"{category}_change-{path1.split('.')[0]}-{path2.split('.')[0]}.png")
...
plt.xlabel(f"{path1.split('.')[0]}")
plt.ylabel(f"{path2.split('.')[0]}")
plt.title('Land Use Transition Matrix')
plt.savefig(f"LUTMat-{path1.split('.')[0]}-{path2.split('.')[0]}.jpg",dpi=300)

import pandas as pd
# 将转移矩阵连同其标签保存到csv中
df_transition_matrix = pd.DataFrame(transition_matrix, index=categories, columns=categories)
df_transition_matrix.to_csv(f"T_mat-{path1.split('.')[0]}-{path2.split('.')[0]}.csv", index=True, encoding='utf-8-sig')
...
plt.title(f"Category Distribution in {path1.split('.')[0]}")
plt.savefig(f"Pie_{path1.split('.')[0]}.jpg",dpi=300)

# 绘制类别计数的饼状图
plt.figure(figsize=(8, 8))
plt.pie(category_count_2, labels=categories, autopct='%1.1f%%', colors=[np.array(color_map[cat])/255 for cat in categories])
plt.title(f"Category Distribution in {path2.split('.')[0]}")
plt.savefig(f"Pie_{path2.split('.')[0]}_Pie.jpg",dpi=300)